...
const getPricingOptions = (product) => {
  if (product.pricing && product.pricing.length > 0) {
    return product.pricing.map(p => ({
      ...p,
      value: p.value || p.tenure // Ensure value exists
    }));
  }
  // Legacy support
  if (product.rentPerDay || product.rentPerWeek) {
    const options = [];
    if (product.rentPerDay) options.push({ tenure: "1 Day", value: "day", price: product.rentPerDay });
    if (product.rentPerWeek) options.push({ tenure: "1 Week", value: "week", price: product.rentPerWeek });
    return options;
  }
  return [];
};
          ...
          <div className="card" style={{ padding: "25px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                {pricingOptions.length > 0 ? (
                  pricingOptions.map((option, index) => (
                    <button
                      key={index}
                      onClick={() => setRentType(option.value)}
                      style={{
                        background: rentType === option.value ? "var(--primary)" : "var(--bg-dark)",
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "20px", paddingTop: "20px", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
              <span style={{ fontSize: "1.2rem", fontWeight: "bold" }}>
                Total: ₹{(pricingOptions.find(p => p.value === rentType)?.price || 0) * duration}
              </span>
              <button
                onClick={() => {
                  if (!rentType) {
                  ...
                  const price = pricingOptions.find(p => p.value === rentType)?.price;
